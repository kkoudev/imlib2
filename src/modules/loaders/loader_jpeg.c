...
{
   return load_scale(im, progress, progress_granularity, immediate_load, 0, 0);
}

char
load_scale(ImlibImage * im, ImlibProgressFunction progress,
     char progress_granularity, char immediate_load,
     int scale_width, int scale_height)
{
   ...
   /* load for scale denom. */
   if ((scale_width > 0) && (scale_height > 0)) {

     w = cinfo.output_width;
     h = cinfo.output_height;

     int denom = w / scale_width;
     if (denom > h / scale_height) {
         denom = h / scale_height;
     }
