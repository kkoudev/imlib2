...
__imlib_ProduceLoader(char *file)
{
   ...
   l->load_scale = dlsym(l->handle, "load_scale");
...
{
   return __imlib_LoadImageForScaleSize(file, progress, progress_granularity, immediate_load, 0, 0, dont_cache, er);
}

ImlibImage         *
__imlib_LoadImageForScaleSize(const char *file, ImlibProgressFunction progress,
                  char progress_granularity, char immediate_load,
                  ...
                  char dont_cache, ImlibLoadError * er)
{
   ...
   if (best_loader) {
      ...
      /* define the load scale function */
      if (best_loader->load_scale) {

        loader_ret =
           best_loader->load_scale(im, progress, progress_granularity, immediate_load, scale_width, scale_height);
        ...
        while ((l) && (im->w == 0))
          {
             ...
             if (l != best_loader) {
                ...
                if (l->load_scale) {
                  ...
                  loader_ret =
                     l->load_scale(im, progress, progress_granularity, immediate_load, scale_width, scale_height);
